...
def tokenize_function(tokenizer, input_field):
    return lambda examples: tokenizer(
        examples[input_field], padding="max_length", truncation=True
    )


# Load and test CANINE
canine_tokenizer = CanineTokenizer.from_pretrained("google/canine-s")
canine = CanineForSequenceClassification.from_pretrained(
    "google/canine-s", num_labels=2
)
...
tokenized_datasets = dataset.map(
    tokenize_function(tokenizer, dataset_input_field), batched=True
)
...
for i in range(0, len(test_dataset), 10):
    small_dataset = HuggingFaceDataset(
        test_dataset.select(list(range(i, min(i + 10, len(test_dataset)))))
    )
